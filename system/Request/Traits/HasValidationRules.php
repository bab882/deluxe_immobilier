...
{
    // On va gerer toute nos validations de formulaire
    ...
    protected function maxNumber($name, $count)
    {
        if($this->checkFieldExist($name))
        {
            ...
            {
                $this->setError($name, "$name min number equal or upper than $count caracter");
            }
        }  
    }
    ...
    public function unique($name, $table, $field = 'id')
    {
        if($this->checkFieldExist($name))
        {
            if($this->checkFirstError($name))
            {
                ...
                if($result != 0)
                {
                    $this->setError($name, "$name most be unique");
                }
            }
        }
    }
    ...
    protected function confirm($name){
        ...
        if($this->checkFieldExist($name))
        {
            ...
            {
                $this->setError($name, " $name confirmation dosn't match");
            } 
        }
    }
